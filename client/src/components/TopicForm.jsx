...
function TopicForm({ setResult, setLoading, loading, setError }) {
  ...
  const handleSubmit = async () => {
    ...
    if (userData && typeof userData.credits === "number" && userData.credits < 10) {
      setError("Insufficient credits! You need at least 10 credits to generate notes.");
      return;
    }
    ...
    try {
      ...
      const result = await generateNotes({topic,
       ...
       console.log(error)
       const serverMessage = error.response?.data?.message || error.message || "Failed to fetch notes from server";
